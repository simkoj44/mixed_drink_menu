...
const BrowseCocktails = (props) => {
  // Variable to store the full list of drinks from the database
  const [fullDrinkList, setFullDrinkList] = useState([]);
  // Variable to store a copy of fullDrinkList (to preserve original list while filtering the state variable above)
  ...
  useLayoutEffect(() => {
    ...
    for (let property in props.drinkObject) {
      // Create 2D array with drink name and alcohol content (so we can later sort by alcohol content)
  ...
  return (
      <div className='browseCocktails'>
        ...
        <div>
          {
            // Iterate through fullDrinkList and display corresponding details
